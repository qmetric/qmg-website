...
def deploy(stack, version=None):
    ...
    files = [('qmetric', '/var/www/html/'),
             ('tech-blog', '/var/www/html/'),
    ]
    service = 'httpd'

    fabric.deploy_app(stack=stack, artifact="qmetric", files=files, templates=templates, version=version, StopService(service), StartService(service))
